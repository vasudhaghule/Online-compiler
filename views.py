    ...
    if request.method == "POST":
        ...
        if(custom):
            ...
            with open(s, "r") as inp:
                ...
                error_code = views.execute(
                    request.user.username, inp, language)
                # print(f"error code: {error_code}")
                f2 = open(
                    "sandbox/submissions/{}/result.txt".format(
            ...
            for tc in testcase.objects.filter(q_id=pk):
                tc_count += 1
                error_code = views.execute(
                    request.user.username, tc.tc_input, language)
                if error_code == 0:
            ...
            if(update_bool == 0):
                ...
                que.save()
            f2 = open(
                "sandbox/submissions/{}/error.txt".format(
                    tester.user.username), "r"
            )
            errors = f2.read()
            errors = san_saf_error(errors, language)
            f2.close()
            ...
            if correct_tcs == tc_count:
                return render(
                    request,
                    "base.html",
                    {"result": "AC", "testcase": cases,
                        'score': scr, 'pk': pk, "time": newtime}
                )
            else:
                return render(
                    request,
                    "base.html",
                    {"result": display_error, "testcase": cases,
                        "error": errors, 'score': scr, 'pk': pk, "time": newtime}
                )
        else:
